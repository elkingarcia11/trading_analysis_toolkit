...
"""
File that generates indicators using existing CSV data
"""
...
def lagging_span(data):
    close_prices = data['Close']

    # Create 26 columns for lagging span, each starting at a different period
    for i in range(26):
        # Slice the close prices to start from the i-th row
        sliced_prices = close_prices.iloc[i:]
        # Calculate lagging span for the current period
        lagging_span = sliced_prices.shift(-26)
        # Add lagging span column to the DataFrame
        data[f'Lagging_Span_{i}'] = lagging_span
...
def generateIndicators(ticker_symbol):
...
def printLastX(ticker_symbol,x):
    # Read the CSV data
    data = pd.read_csv(f"data/{ticker_symbol}_WITH_INDICATORS.csv")

    # Extract the "Lagging Span" column
    lagging_span = data["Lagging_Span"]

    # Print the last 10 rows of the "Lagging Span" column
    print("Last 10 rows of Lagging Span:")
    print(lagging_span.tail(x))

"""
ticker_symbol = "BTC"
generateIndicators(ticker_symbol)
printLastX(ticker_symbol,30)
"""
